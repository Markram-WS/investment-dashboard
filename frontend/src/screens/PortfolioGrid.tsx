...
const PortfolioGrid: React.FC = () => {
  ...
  React.useEffect(() => {
    if (selectedPortfolio) {
      api
        .getPerformance(selectedPortfolio.portfolio_id)
  ...
  React.useEffect(() => {
    if (selectedPortfolio) {
      api
        .getTradeHistory(selectedPortfolio.portfolio_id)
  ...
  const { renderMarkdown, renderRiskStatus, isGridType } =
    useMarkdownRenderer();
  ...
  const totalCash =
    (selectedPortfolio?.available_cash || 0) +
    (selectedPortfolio?.money_market || 0);
    ...
    <div className="px-10 py-6 min-h-screen max-w-[1800px] mx-auto">
      ...
      <header className="flex justify-between items-center w-full mb-6">
        <div className="flex flex-col">
          ...
          <h2 className="text-3xl font-bold tight-headline">
            {selectedPortfolio?.portfolio_name} Overview
          </h2>
        ...
        <div className="col-span-12 lg:col-span-8 bg-white rounded-xl border border-hairline p-8 flex flex-col">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8 pb-8 border-b border-hairline">
            ...
            <div className="flex flex-col">
              ...
              <p
                className={`text-3xl font-bold leading-none ${cumulativePl >= 0 ? "text-brand-teal" : "text-brand-coral"}`}
              >
            ...
            <div className="flex flex-col">
              ...
              <p className="text-3xl font-bold leading-none text-ink">
                ${Math.max(0, availableCash).toLocaleString()}
              </p>
              <p className="text-slate text-xs mt-2">Liquid after deductions</p>
            </div>
          </div>
          <div className="mb-8 pb-8 border-b border-hairline">
            <p className="text-[11px] font-bold text-gray-500 mb-4 uppercase tracking-widest">
              Cash Details
            </p>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              ...
              <div>
                ...
                <p className="text-lg font-bold">${(selectedPortfolio?.money_market || 0).toLocaleString()}</p>
              </div>
              <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-0">
            <div className="flex border-r border-hairline pr-8">
              <div className="flex flex-col w-full">
                <p className="text-[11px] font-bold text-gray-500 mb-6 uppercase tracking-widest">
                  Risk Level
                </p>
                  ...
                  <div className="flex flex-col gap-3 items-start">
                    <div className="flex items-center gap-2 bg-surface px-3 py-1.5 rounded-full border border-hairline">
                      ...
                      <span className="text-[11px] font-bold uppercase tracking-wider">
                        {selectedPortfolio?.risk_status || "Moderate"}
                      </span>
                    </div>
                    <p className="text-[11px] text-slate font-medium max-w-[120px]">
                      Score based on portfolio volatility
                    </p>
                  </div>
                </div>
              </div>
          ...
          <TagsSection tags={selectedPortfolio?.tags} />
        </div>

        {/* Strategy & Notes */}
        <div className="col-span-12 lg:col-span-4 rounded-2xl bg-yellow-100 p-8 flex flex-col border border-hairline-soft">
          <h3 className="text-[11px] font-bold text-ink uppercase tracking-widest mb-6">
            Strategy & Notes
          </h3>
          <div className="space-y-6">
            ...
            <div className="pt-6 border-t border-ink/10">
              <label className="text-[10px] font-bold text-gray-500 block mb-2 uppercase tracking-wider">
                Internal Notes
              </label>
              {editingNotes ? (
                <div>
                ...
                <div
                  ...
                  onClick={() => {
                    setNotesContent(selectedPortfolio?.internal_notes || "");
          ...
          <div className="flex items-center gap-6">
            {isGridType(selectedPortfolio) && (
              <div className="flex items-center gap-2">
                ...
                >
                  <span
                    className={`inline-block h-3 w-3 transform rounded-full bg-white transition-transform ${groupByZone ? "translate-x-4" : "translate-x-0"}`}
                  />
                </button>
              </div>
            )}
              ...
              <tbody>
                {activeOrders.map((order) => {
                  ...
                  return (
                    <tr
                      key={order.order_id}
                      className="border-b border-hairline-soft bg-white hover:bg-surface/50"
                    >
                      ...
                      <td className="px-3 py-2">
                        <span
                          className={`text-xs px-1 rounded ${order.side === "BUY" || order.side === "Buy" ? "bg-teal-light text-brand-teal" : "bg-coral-light text-brand-coral"}`}
                        >
                          {order.side}
                        </span>
                      </td>
                      ...
                      <td className="px-3 py-2">
                        <span
                          className={`text-xs ${order.order_status === "filled" ? "text-green-600" : "text-gray-500"}`}
                        >
                          {order.order_status}
                        </span>
                      </td>
              ...
              {tradeHistory.length === 0 ? (
                ...
                <table className="w-full text-xs">
                  <thead className="bg-gray-50 sticky top-0">
                    <tr>
                      <th className="px-3 py-1.5 text-left font-medium text-[10px] uppercase tracking-wider">
                        Open ID
                      </th>
                      <th className="px-3 py-1.5 text-left font-medium text-[10px] uppercase tracking-wider">
                        Close ID
                      ...
                      <th className="px-3 py-1.5 text-right font-medium text-[10px] uppercase tracking-wider">
                        Qty
                      </th>
                      <th className="px-3 py-1.5 text-right font-medium text-[10px] uppercase tracking-wider">
                        Entry
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-3 py-1.5">
                            <span
                              className={`px-1 rounded text-[9px] font-bold ${h.type === "BUY" || h.type === "Buy" ? "bg-teal-light text-brand-teal" : "bg-coral-light text-brand-coral"}`}
                            >
                              {h.type}
                            </span>
                          </td>
                          ...
                          <td className="px-3 py-1.5 text-right">
                            {h.entry_price != null
                              ? `$${Number(h.entry_price).toLocaleString()}`
                              : "-"}
                          </td>
                          <td className="px-3 py-1.5 text-right">
                            {h.exit_price != null
                              ? `$${Number(h.exit_price).toLocaleString()}`
                              : "-"}
                          </td>
                          <td
                            className={`px-3 py-1.5 text-right font-medium ${(h.realized_pl ?? 0) >= 0 ? "text-green-600" : "text-red-500"}`}
                          >
                            {h.realized_pl != null
                              ? `$${Number(h.realized_pl).toLocaleString()}`
                              : "-"}
                          </td>
...
const PerformanceChart: React.FC<{
  ...
  if (viewMode === "equity") {
    ...
    const toX = (i: number) =>
      PAD + (W - 2 * PAD) * (i / (points.length - 1 || 1));

    const pathD = points
      .map(
        (p: any, i: number) =>
          `${i === 0 ? "M" : "L"}${toX(i)},${toY(p.cumulative_pl)}`,
      )
      .join(" ");
...
const TagsSection: React.FC<{ tags: Record<string, boolean> | null }> = ({ tags }) => {
